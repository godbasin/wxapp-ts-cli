 ...
 * @return {Date}
 */
export function str2Date(date, p) {
  if (!date) {
    return date;
  }
  p = p || "%Y-%M-%d %h:%m:%s";
 ...
 * @return {String}
 */
export function date2Str(date, p, isFill) {
  if (!date) {
    return date;
  }
 ...
 * @return {Float}
 */
export function dateDiff(d1, d2, cmpType) {
  var diff = 0;
  switch (cmpType) {
    case "Y":
 ...
 */
export function dateAdd(interval, indate, offset) {
  switch (interval) {
    case "y":
 ...
 */
export function leapYear(indate) {
  var _days = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  var _is = false;
  var _d = 365;

  if (
    (indate.getFullYear() % 4 === 0 && indate.getFullYear() % 100 !== 0) ||
    indate.getFullYear() % 400 === 0
  ) {
    _days.splice(1, 1, 29);
    _is = true;
    _d = 366;
 ...
 * @param {Date} date_start 开始日期
 * @param {Date} date_end 结束日期
 * @param {Date} closed_nterval 是否是闭区间
 * @return {Boolean}
 */
export function dateBetween(date, date_start, date_end, closed_nterval) {
  if (closed_nterval) {
    return (
      date.getTime() >= date_start.getTime() &&
      date.getTime() <= date_end.getTime()
    );
  } else {
    return (
      date.getTime() > date_start.getTime() &&
